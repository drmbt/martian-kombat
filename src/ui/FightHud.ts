...
export class FightHud {
  ...
  constructor(
    ...
    // fills the UiLayer root, which is pinned over the game canvas
    this.root = document.createElement('div');
    this.root.style.cssText =
      'position:absolute;inset:0;pointer-events:none;color:#e8e4d8;font:12px monospace;' +
  ...
  update(s: GameState, frame: FightHudFrame): void {
    ...
    const info =
      `[ESC] pause  [F1] hitboxes  [F2] move log  [F3] skeleton  [F4] settings  [F5] inspector  [${frame.tauntKey}] taunt  [F9] restart\n` +
